function solution(passwords, s) {
  var answer = 0;
  const pw_map = new Map();
  let isMatch= "";
  //배열을 맵으로
  passwords.forEach(([key,value]) => {
    pw_map.set(key,value);
  });

  
  while(true){
    //현관과 비밀번호중 하나라도 입력 안될경우 마지막에 누르다 만 것으로 간주 탈출
    if (!s.match(/(\d+\#\d+\#)/))
      break;
    //정규표현식으로 #이 두번 나올때까지 문자열 파싱
    let temp = s.match(/(\d+\#\d+\#)/);
    s= s.replace(/(\d+\#\d+\#)/,"");
    isMatch = (temp[0].split(/\#/));
    //키 밸류 비교
    if(pw_map.get(Number(isMatch[0])) === Number(isMatch[1])){
      answer+=1;
    }
